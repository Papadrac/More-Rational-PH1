...
if st.session_state.submitted_last:
    ...
    with row[1]:
        if st.button("Suriin Muli"):
            ...
            st.rerun()
    with row[2]:
        st.markdown("<a href='https://morerationalph.streamlit.app/Set_Theory-Complement_of_a_set' target='_self'><button style='font-size:16px;padding:8px 16px;border-radius:6px;background:#0099f6;color:white;border:none;cursor:pointer;'>Pumunta sa Susunod na Seksyon →</button></a>", unsafe_allow_html=True)
    st.stop()
